    ...
    if (root) {
        if (is_leaf(root)) {
            ...
            *trash_size += (root->frequency * depth);
            ...
            mapping <<= 1;
            bytes_mapping(root->left, hash_table, compressed_file, mapping, depth + 1, trash_size);
            mapping++;
            bytes_mapping(root->right, hash_table, compressed_file, mapping, depth + 1, trash_size);
        }
    }
}
